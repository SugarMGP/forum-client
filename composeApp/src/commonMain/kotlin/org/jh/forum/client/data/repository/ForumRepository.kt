...
class ForumRepository(private val api: ForumApi) {
    ...
    suspend fun getProfile(id: Long? = null): AjaxResult<GetUserProfileResponse> = api.getProfile(id)
    suspend fun updateProfile(request: UpdateUserProfileRequest): AjaxResultVoid = api.updateMyProfile(request)
    ...
    suspend fun createPost(request: PublishPostRequest): AjaxResult<ModerationResultResponse> = api.createPost(request)
    suspend fun getPostList(
        page: Int,
        pageSize: Int,
        category: String?,
        sortType: String?
    ): AjaxResult<BaseListResponse<GetPostListElement>> = api.getPostList(page, pageSize, category, sortType)
    ...
    ): AjaxResult<GetCommentReplyListResponse> = api.getReplyList(page, pageSize, id, highlightReplyId)
    ...
    suspend fun getAnnouncementList(
        ...
        pageSize: Int,
        type: String?
    ): AjaxResult<BaseListResponse<GetAnnouncementListElement>> = api.getAnnouncementList(page, pageSize, type)

    suspend fun reportUser(request: ReportUserRequest): AjaxResultVoid = api.reportUser(request)
    suspend fun reportContent(request: ReportContentRequest): AjaxResultVoid = api.reportContent(request)
    ...
    suspend fun getPersonalPostList(
        ...
        pageSize: Int,
        userId: Long?
    ): AjaxResult<BaseListResponse<GetPersonalPostListElement>> = api.getPersonalPostList(page, pageSize, userId)
